...
void getUserInfo(struct user_detail *userInfo)
{
    ...
    do
    {
        printf("Enter Date borrowed (MMDD): ");
...
int main()
{
    int key = 1, week = 0, fine = 0, day = 1, dayKept = 0;
    char checker = 'n';
    ...
    while (1)
    {
        ...
        {
            printf("\n%-30s %-30s %-30s %-20s %-20s %-10s %-10s \n", "BorrowerBook", "titleBook", "AuthorName", "BorrowedDate", "ReturnedDate", "Fine", "Daykept");
            printf("\n%-30s %-30s %-30s %-10s %-10d %-10s %-10d %-10d %-10d", userInfo.borrowerName, userInfo.bookTitle, userInfo.authorName, monthList[month], days, monthList[month], days, fine, dayKept);
            break;
        }
        else {
